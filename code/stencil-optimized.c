...
int get_numa_node_of_current_thread() {
...
void create_dist_grid(dist_grid_info_t *grid_info, int stencil_type) {
    ...
    /* The following is an example of how to use the NUMA library, modify it on your own */
    data_t **numa_memories = NULL;
    int num_numas = get_total_numa_nodes();
    int numa = get_numa_node_of_current_thread();
    size_t size = grid_info->local_size_x
                * grid_info->local_size_y
                * (grid_info->local_size_z / num_numas);
    numa_memories = (data_t **)calloc(num_numas, sizeof(data_t *));
    numa_memories[numa] = numa_alloc_onnode(sizeof(data_t) * size, numa);
    grid_info->additional_info = (void *)numa_memories;
    #pragma omp parallel
    {
        /* ... do something with the numa_memory */
        // int num_threads = omp_get_num_threads();
        // int thread = omp_get_thread_num();
        // int cpu = sched_getcpu();
        // int numa = get_numa_node_of_current_thread();
        // printf("Thread %d on CPU %d and NUMA %d\n", thread, cpu, numa);
        // data_t *numa_memory = numa_memories[numa];
    }
}

void destroy_dist_grid(dist_grid_info_t *grid_info) {
    /* The following is an example of how to use the NUMA library, modify it on your own */
    data_t **numa_memories = (data_t **)grid_info->additional_info;
    int num_numas = get_total_numa_nodes();
    for (int numa = 0; numa < num_numas; ++numa) {
        if (numa_memories[numa] != NULL) {
            size_t size = grid_info->local_size_x
                        * grid_info->local_size_y
                        * (grid_info->local_size_z / num_numas);
            numa_free(numa_memories[numa], sizeof(data_t) * size);
        }
    }
    free(numa_memories);
}
